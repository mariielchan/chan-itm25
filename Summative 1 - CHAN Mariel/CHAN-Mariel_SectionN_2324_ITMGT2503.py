...
for k, v in jsondata.items():
    ...
    if piece1 in str(v).upper():
        modified = str(v).upper().replace(piece1,"")
        print(modified)

    if piece2 in modified:
        modified2 = modified.replace(piece2,"")
        print(modified2)

    if piece3 in modified2:
        modified3 = modified2.replace(piece3,"")
        print(modified3)

    if piece4 in modified3:
        modified4 = modified3.replace(piece4,"")
        print(modified4)

    if piece5 in modified4:
        modified5 = modified4.replace(piece5,"")
        print(modified5)

    if piece6 in modified5:
        modified6 = modified5.replace(piece6,"")
        print(modified6)

    if char in unnecessary_punctuation in str(v).upper():
        modify = modified6.replace(char," ")
        print(modify)
...
optimal_F = max(F * (1000 +  500 + 50))
optimal_R = R * (600 + 750 + 45)
optimal_C = C * (750 + 300 + 75)

                
optimal_Z = max(10*optimal_F) + max(12*optimal_R) + max(16*optimal_C)
...
def Collatz(start_number):
    ...
    if start_number > 0:
        ...
        if start_number % 2 != 0:
            collatz_sequence.append(start_number)
            odd = (start_number * 3) + 1
            if odd % 2 == 0:
                collatz_sequence.append(odd)
                odd = odd // 2
                collatz_sequence.append(odd)
                odd = odd // 2
                collatz_sequence.append(odd)
    ...
    my_dict = {'collatz_sequence':collatz_sequence, 'collatz_length':collatz_length, 'max_collatz':max_collatz, 'sequence_sum':sequence_sum}    
